# Processo da compra

## Estado atual

- Estado: pesquisando
- Proxima acao: confirmar manualmente preco, frete, estoque, vendedor e garantia dos finalistas
- Decisao aberta: DPS Clamper Front 20kA 175V lidera a pesquisa web, mas ainda nao fecha compra sem cotacao manual.

## Linha do tempo decisoria

| Data | Etapa | Decisao | Por que |
|---|---|---|---|
| 2026-09-21 | cotacao | Este projeto fica so com o DPS; disjuntor foi para 2026-disjuntor-bipolar-motor-portao | Itens complementares nao podem competir no mesmo ranking (regra 3). |
| 2026-09-21 | cotacao | Cotacao registrada para clamper-front-20ka-175v | Amazon / fonte=web / custo_total=83.8 |
| 2026-09-21 | produto | Candidato registrado: DPS Clamper Front 20kA 175V | id=clamper-front-20ka-175v |
| 2026-09-21 | modelo | Disjuntor bipolar 10A C + 2 DPS classe II | Circuito proprio 220V; tensao do DPS depende de fase-fase x fase-neutro, confirmar com Fabio antes de comprar. |
| 2026-09-21 | acessorio | acessorio de 2026-motor-portao-deslizante | comprado a parte para comparar preco sem misturar ranking |
| 2026-09-21 | abertura | Compra criada | Necessidade registrada no briefing |

## Etapas

- [ ] 1. Definir o modelo e a especificacao tecnica para o meu contexto (com acessorios)
- [ ] 2. Mapear candidatos
- [x] 3. Registrar produtos em `produtos/`
- [x] 4. Coletar cotacoes iniciais
- [x] 5. Aplicar gates eliminatorios
- [ ] 6. Comparar finalistas
- [ ] 7. Confirmar preco/frete/estoque manualmente
- [ ] 8. Registrar decisao e por que os outros perderam
- [ ] 9. Comprar
- [ ] 10. Fazer veredito D+30
- [ ] 11. Fazer veredito D+180

## O que ja sei

- 

## O que ainda preciso descobrir

- 

## Hipoteses descartadas

| Hipotese | Motivo do descarte |
|---|---|
